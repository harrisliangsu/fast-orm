...
public class EntityUtil {
    ...
    public static Map<String, Object> getNotNullField(Entity entity) {
        ...
        addNotNullField(entity, fields,false);
        return fields;
    }
     ...
     * @return HashMap contained not null fields
     */
    public static Map<String, Object> getNotNullFieldDb(Entity entity) {
        Map<String, Object> fields = Maps.newHashMap();
        addNotNullField(entity, fields,true);
        return fields;
    }
     ...
     * @param fields a map
     */
    public static void addNotNullField(Entity entity, Map<String, Object> fields, boolean originalDbValue) {
        EntityInfo entityInfo = FeiFeiBootStrap.get().<FeiFeiContainer>container().getEntityInfoGet().get(entity);
        for (String fieldName : entityInfo.getFieldInfo().keySet()) {
            Method methodGet = entityInfo.getMethodInfo().get(methodGet(fieldName));
            try {
                Object fieldValue = methodGet.invoke(entity);
                // Whether field type is StatusCode or Status
                if (originalDbValue){
                    fieldValue = mapFieldValueToDbValue(fieldValue);
                }
                if (fieldValue != null) {
    ...
    public static Object mapFieldValueToDbValue(Object fieldValue) {
        if (fieldValue == null) return null;
        if (Status.class.isAssignableFrom(fieldValue.getClass())) {
            return ((Status) fieldValue).getName();
        } else if (StatusCode.class.isAssignableFrom(fieldValue.getClass())) {
            return ((StatusCode) fieldValue).getCode();
        } else {
            return fieldValue;
        }
    }
    ...
    public static String getNotNullFirstFieldName(Entity entity) {
        return getNotNullField(entity).keySet().stream().findFirst().orElse(null);
    }
